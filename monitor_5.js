...
try {
    var configuration = require('./config_5.json');
} catch (e) {
    api.log('error', 'Missing, config_5.json file or invalid json syntax.')
    return process.exit()
}
...
if (configuration.slackBot.active) {
    ...
    slackBot.on('start', function() {
        slackBot.postMessageToChannel(configuration.slackBot.channel, 'PLUG 5 IS COOKING ◕‿◕', configuration.slackBot.settings);
    })
