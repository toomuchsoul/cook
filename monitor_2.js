...
try {
    var configuration = require('./config_2.json');
} catch (e) {
    api.log('error', 'Missing, config_2.json file or invalid json syntax.')
    return process.exit()
}
...
if (configuration.slackBot.active) {
    ...
    slackBot.on('start', function() {
        slackBot.postMessageToChannel(configuration.slackBot.channel, 'PLUG 2 IS COOKING ◕‿◕', configuration.slackBot.settings);
    })
