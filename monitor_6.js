...
try {
    var configuration = require('./config_6.json');
} catch (e) {
    api.log('error', 'Missing, config_6.json file or invalid json syntax.')
    return process.exit()
}
...
if (configuration.slackBot.active) {
    ...
    slackBot.on('start', function() {
        slackBot.postMessageToChannel(configuration.slackBot.channel, 'PLUG 6 IS COOKING ◕‿◕', configuration.slackBot.settings);
    })
