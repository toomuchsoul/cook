...
try {
    var configuration = require('./config_1.json');
} catch (e) {
    api.log('error', 'Missing, config_1.json file or invalid json syntax.')
    return process.exit()
}
...
if (configuration.slackBot.active) {
    ...
    slackBot.on('start', function() {
        slackBot.postMessageToChannel(configuration.slackBot.channel, 'PLUG 1 IS COOKING ◕‿◕', configuration.slackBot.settings);
    })
